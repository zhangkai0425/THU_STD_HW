...
def test_task3(video_path,result_path):
    ...
    for file_name in os.listdir(video_path):
        ...
        audio_left   = audio_trace[:,:1] + np.random.random(audio_trace[:,:1].shape)*0.00001
        audio_middle = audio_trace[:,:1] + np.random.random(audio_trace[:,:1].shape)*0.00001
        audio_right  = audio_trace[:,:1] + np.random.random(audio_trace[:,:1].shape)*0.00001
